<template>
	<section id="contacto" class="bg-white pt-20 pb-16">
		<UContainer>

			<!-- Encabezado -->
			<div class="mb-8">
				<h3 class="uppercase text-5xl md:text-6xl text-stone-400">Contacto</h3>
			...
			<div class="flex flex-wrap justify-center mb-16 gap-4">
				...
				<div class="w-full md:w-1/3">
					...
					<p class="my-2 text-stone-600">
						O envía un correo a
						<a href="mailto:HerramientasAltaCalidad@hotmail.com" class="italic hover:text-red-600 transition-colors">
							HerramientasAltaCalidad@hotmail.com
						</a>
					</p>
					<p class="text-stone-600">Por favor completa el formulario a continuación:</p>

					<form class="mt-4 flex flex-col gap-3" @submit.prevent="enviarFormulario">
					    <UInput v-model="nombre" type="text" placeholder="Nombre*" size="lg" :ui="{ base: 'bg-white text-stone-900' }" />
					    <UInput v-model="email" type="email" placeholder="Email*" size="lg" :ui="{ base: 'bg-white text-stone-900' }" />
					    <UInput v-model="asunto" type="text" placeholder="Asunto" size="lg" :ui="{ base: 'bg-white text-stone-900' }" />
					    <UTextarea v-model="mensaje" placeholder="Mensaje" :rows="5" :ui="{ base: 'bg-white text-stone-900 resize-none' }" />

					    <!-- Feedback -->
					    <p v-if="enviado" class="text-green-600 text-sm">✓ Mensaje enviado. Te responderemos a la brevedad.</p>
					    <p v-if="errorEnvio" class="text-red-600 text-sm">Ocurrió un error. Intenta de nuevo o llámanos directamente.</p>

					    <UButton
					        type="submit"
					        :loading="enviando"
					        :disabled="enviando"
					        class="bg-red-600 hover:bg-stone-900 text-white px-6 py-2 rounded-xl transition-colors duration-200 w-fit"
					    >
					        Enviar
					    </UButton>
					</form>
				...
				<!-- Info de contacto -->
				<div class="w-full md:w-1/3">
					<h5 class="text-3xl my-4 text-stone-900">Centro de distribución</h5>
					<address class="not-italic text-stone-600">
						Av. La Paz 1181 Col. Centro CP: 44100, Guadalajara, Jalisco.
					</address>
...
async function enviarFormulario() {
    ...
    try {
        await $fetch('/api/mail/contact', {
            method: 'POST',
            body: { nombre: nombre.value, email: email.value, asunto: asunto.value, mensaje: mensaje.value },
        })
        enviado.value = true
        nombre.value = ''
        email.value = ''
