		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
			...
			<UCard :ui="{ body: { padding: 'p-4' } }">
				<div class="flex flex-col sm:flex-row gap-3">
					<div class="flex-1">
						<UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Buscar productos..." :ui="{ wrapper: 'w-full' }"/>
					</div>
					<div class="flex gap-2">
						<UButton color="gray" variant="outline" icon="i-heroicons-arrow-path" :loading="refreshing" @click="handleRefresh">Actualizar</UButton>
					</div>
				</div>
			</UCard>
			...
			<UCard :ui="{ body: { padding: 'p-0' } }">
				...
				<div class="px-4 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
					...
					<UBadge v-if="filteredProducts.length !== products?.length" color="primary" variant="subtle" size="xs">{{ filteredProducts.length }} resultado{{ filteredProducts.length !== 1 ? 's' : '' }}</UBadge>
				</div>
				...
				<div v-if="filteredProducts.length === 0" class="flex flex-col items-center justify-center py-16 px-4">
					...
					<UButton v-else color="gray" variant="ghost" size="sm" class="mt-4" @click="search = ''">Limpiar búsqueda</UButton>
				</div>

				<!-- Table -->
				<UTable v-else :data="filteredProducts" :columns="columns" :ui="{tr: { base: 'hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors' }, th: { base: 'bg-gray-50 dark:bg-gray-800/50 text-xs uppercase tracking-wider' }, td: { base: 'text-sm' }}">
				...
				<!-- Table Footer -->
				<div v-if="filteredProducts.length > 0" class="px-4 py-3 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between">
					<p class="text-xs text-gray-500 dark:text-gray-400">Mostrando <span class="font-medium">{{ filteredProducts.length }}</span> de <span class="font-medium">{{ products?.length ?? 0 }}</span> productos</p>
				</div>	
			</UCard>
		</div>
