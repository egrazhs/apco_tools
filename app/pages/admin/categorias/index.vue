  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-1">
            ...
            <span class="text-gray-900 dark:text-white font-medium">Categorías</span>
          </div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Categorías</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {{ categories?.length ?? 0 }} categoría{{ (categories?.length ?? 0) !== 1 ? 's' : '' }} registrada{{ (categories?.length ?? 0) !== 1 ? 's' : '' }}
          </p>
        </div>
        <UButton
      ...
      <UCard :ui="{ body: { padding: 'p-0' } }">
        ...
        <!-- Empty State -->
        <div v-if="filteredCategories.length === 0" class="flex flex-col items-center justify-center py-16 px-4">
        ...
        >
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Mostrando <span class="font-medium">{{ filteredCategories.length }}</span> de <span class="font-medium">{{ categories?.length ?? 0 }}</span> categorías
          </p>
    ...
    <UModal v-model:open="deleteModalOpen">
      <template #header>
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Eliminar Categoría</h3>
