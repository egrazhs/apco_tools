    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <UButton icon="i-heroicons-plus" size="md" class="self-start sm:self-auto" @click="navigateTo('/admin/subcategorias/create')">
                    Nueva Subcategoría
                </UButton>
            </div>
            ...
            <UCard :ui="{ body: { padding: 'p-4' } }">
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1">
                        <UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Buscar categorías..." :ui="{ wrapper: 'w-full' }" />
                    </div>
            ...
            <UCard :ui="{ body: { padding: 'p-0' } }">
                ...
                <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
					...
					<UBadge v-if="filteredSubcategories.length !== subcategories?.length" color="primary" variant="subtle" size="xs" >
				...
				<div v-if="filteredSubcategories.length === 0" class="flex flex-col items-center justify-center py-16 px-4">
					...
					<UButton v-if="!search && !filterEstado" icon="i-heroicons-plus" size="sm" class="mt-4" @click="navigateTo('/admin/subcategorias/create')">
						Agregar Subcategoría
					</UButton>
					
					<UButton v-else color="gray" variant="ghost" size="sm" class="mt-4" @click="clearFilters">
						Limpiar filtros
					</UButton>
				</div>
				...
				>
					...
					<template #is_active-cell="{ row }">
						<UBadge :color="row.original.is_active ? 'green' : 'red'" variant="subtle" class="gap-1.5">
							<span class="w-1.5 h-1.5 rounded-full inline-block" :class="row.original.is_active ? 'bg-green-500' : 'bg-red-500'" />
				...
				<!-- Table Footer -->
				<footer v-if="filteredSubcategories.length > 0" class="px-4 py-3 border-t border-gray-200 dark:border-gray-800 flex items-center justify-between">
